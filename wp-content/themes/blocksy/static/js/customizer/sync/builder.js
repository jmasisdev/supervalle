...
const makeShortcutFor = item => {
   ...
   M17,12c0,1.1,0.9,2,2,2s2-0.9,2-2s-0.9-2-2-2S17,10.9,17,12z"/></svg>
		...
		if (
			item.dataset.location.indexOf('header:builder_panel_top-row') > -1
		) {
			text = 'Top Row'
		}

		if (
			item.dataset.location.indexOf('header:builder_panel_middle-row') >
			-1
		) {
			text = 'Main Row'
		}
		...
		if (
			item.dataset.location.indexOf('header:builder_panel_bottom-row') >
			-1
		) {
			text = 'Bottom Row'
		}
