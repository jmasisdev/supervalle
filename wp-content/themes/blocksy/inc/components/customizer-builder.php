...
class Blocksy_Customizer_Builder {
	...
	public function dynamic_css($panel_type = 'header', $args = []) {
		...
		if (
			$panel_type === 'header'
			&&
			! blocksy_manager()->header_builder->enabled_on_this_page()
		) {
			return;
		}
		...
		if (
			$panel_type === 'footer'
			&&
			! blocksy_manager()->footer_builder->enabled_on_this_page()
		) {
			return;
		}
		...
		foreach ($all_items as $item) {
			...
			blocksy_theme_get_dynamic_styles(apply_filters(
				'blocksy:' . $panel_type . ':dynamic-styles-args',
				array_merge([
		...
		if ($panel_type === 'header') {
			...
			if (file_exists($path)) {
				blocksy_theme_get_dynamic_styles(apply_filters('blocksy:header:dynamic-styles-args', array_merge([
					'path' => $path
				], $args)));
			}
		}
